...
import com.hypixel.hytale.component.system.data.EntityDataSystem.SystemTaskData;
...
public class Store<ECS_TYPE> implements ComponentAccessor<ECS_TYPE> {
  ...
  static {
    METRICS_REGISTRY =
        (new MetricsRegistry()).register("ArchetypeChunkCount", Store::getArchetypeChunkCount, Codec.INTEGER)
            .register("EntityCount", Store::getEntityCount, Codec.INTEGER).register("Systems", (componentStore) -> {
              ...
              for (int systemIndex = 0; systemIndex < data.getSystemSize(); ++systemIndex) {
                ...
                systemMetricData[systemIndex] =
                    new SystemMetricData(system.getClass().getName(), componentStore.getArchetypeChunkCountFor(systemIndex),
                        componentStore.getEntityCountFor(systemIndex),
                        system instanceof TickingSystem ? systemMetrics[systemIndex] : null, metrics);
              }

              return systemMetricData;
            }, new ArrayCodec(SystemMetricData.CODEC, (x$0) -> new SystemMetricData[x$0]))
            .register("ArchetypeChunks", Store::collectArchetypeChunkData,
                new ArrayCodec(ArchetypeChunkData.CODEC, (x$0) -> new ArchetypeChunkData[x$0]));
  }
  ...
  private final ParallelTask<EntityTickingSystem.SystemTaskData<ECS_TYPE>> parallelTask =
      new ParallelTask(EntityTickingSystem.SystemTaskData::new);
  @Nonnull
  ...
  private final ParallelTask<EntityDataSystem.SystemTaskData<ECS_TYPE, ?, ?>> fetchTask =
      new ParallelTask(EntityDataSystem.SystemTaskData::new);
  @Nonnull
  ...
  Store(@Nonnull ComponentRegistry<ECS_TYPE> registry, int storeIndex, @Nonnull ECS_TYPE externalData,
  ...
  void onAdd(@Nonnull ComponentRegistry.Data<ECS_TYPE> data) {
    ...
    for (int systemIndex = -1;
         (systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0; this.systemMetrics[systemIndex] =
             HistoricMetric.builder(33333333L, TimeUnit.NANOSECONDS).addPeriod(1L, TimeUnit.SECONDS)
                 .addPeriod(1L, TimeUnit.MINUTES).addPeriod(5L, TimeUnit.MINUTES).build()) {
    }
  ...
  @Nullable
  public Ref<ECS_TYPE> addEntity(@Nonnull Holder<ECS_TYPE> holder, @Nonnull Ref<ECS_TYPE> ref,
  ...
  @Nonnull
  public Ref<ECS_TYPE>[] addEntities(@Nonnull Holder<ECS_TYPE>[] holders, int start, int length,
                                     @Nonnull AddReason reason) {
  ...
  public void addEntities(@Nonnull Holder<ECS_TYPE>[] holders, @Nonnull Ref<ECS_TYPE>[] refs,
                          @Nonnull AddReason reason) {
  ...
  public void addEntities(@Nonnull Holder<ECS_TYPE>[] holders, int holderStart, @Nonnull Ref<ECS_TYPE>[] refs,
                          int refStart, int length, @Nonnull AddReason reason) {
  ...
  public Holder<ECS_TYPE> copySerializableEntity(@Nonnull Ref<ECS_TYPE> ref, @Nonnull Holder<ECS_TYPE> holder) {
    ...
    int refIndex = ref.getIndex();
    int archetypeIndex = this.entityToArchetypeChunk[refIndex];
    return this.archetypeChunks[archetypeIndex].copySerializableEntity(this.registry.getData(),
        this.entityChunkIndex[refIndex], holder);
  }
  ...
  @Nonnull
  public Holder<ECS_TYPE> removeEntity(@Nonnull Ref<ECS_TYPE> ref, @Nonnull Holder<ECS_TYPE> holder,
                                       @Nonnull RemoveReason reason) {
    return this.removeEntity(ref, holder, reason, (Throwable) null);
  }

  @Nonnull
  Holder<ECS_TYPE> removeEntity(@Nonnull Ref<ECS_TYPE> ref, @Nonnull Holder<ECS_TYPE> holder,
                                @Nonnull RemoveReason reason, @Nullable Throwable proxyReason) {
    this.assertThread();
    this.assertWriteProcessing();
    ref.validate();
    CommandBuffer<ECS_TYPE> commandBuffer = this.takeCommandBuffer();
  ...
  @Nonnull
  public Holder<ECS_TYPE>[] removeEntities(@Nonnull Ref<ECS_TYPE>[] refs, int start, int length,
                                           @Nonnull RemoveReason reason) {
  ...
  @Nonnull
  public Holder<ECS_TYPE>[] removeEntities(@Nonnull Ref<ECS_TYPE>[] refs, @Nonnull Holder<ECS_TYPE>[] holders,
                                           @Nonnull RemoveReason reason) {
  ...
  @Nonnull
  public Holder<ECS_TYPE>[] removeEntities(@Nonnull Ref<ECS_TYPE>[] refArr, int refStart,
                                           @Nonnull Holder<ECS_TYPE>[] holders, int holderStart, int length,
                                           @Nonnull RemoveReason reason) {
    int refEnd = refStart + length;
    int holderEnd = holderStart + length;
      ...
      try {
        ...
        while ((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
          for (int i = refStart; i < refEnd; ++i) {
            ...
            BitSet entityProcessedBySystemIndexes = this.archetypeChunkIndexesToSystemIndex[archetypeIndex];
            if (entityProcessedBySystemIndexes.get(systemIndex)) {
              ((RefSystem) data.getSystem(systemIndex, systemType)).onEntityRemove(refArr[i], reason, this,
                  commandBuffer);
            }
          }
        }
  ...
  public <T extends Component<ECS_TYPE>> void ensureComponent(@Nonnull Ref<ECS_TYPE> ref,
  ...
  @Nonnull
  public <T extends Component<ECS_TYPE>> T ensureAndGetComponent(@Nonnull Ref<ECS_TYPE> ref,
                                                                 @Nonnull ComponentType<ECS_TYPE, T> componentType) {
    this.assertThread();
    this.assertWriteProcessing();
    int refIndex = ref.getIndex();
  ...
  @Nonnull
  public <T extends Component<ECS_TYPE>> T addComponent(@Nonnull Ref<ECS_TYPE> ref,
                                                        @Nonnull ComponentType<ECS_TYPE, T> componentType) {
    this.assertThread();
    this.assertWriteProcessing();
    T component = this.registry._internal_getData().createComponent(componentType);
  ...
  public <T extends Component<ECS_TYPE>> void addComponent(@Nonnull Ref<ECS_TYPE> ref,
                                                           @Nonnull ComponentType<ECS_TYPE, T> componentType,
                                                           @Nonnull T component) {
    this.assertThread();
    this.assertWriteProcessing();
  ...
  public <T extends Component<ECS_TYPE>> void replaceComponent(@Nonnull Ref<ECS_TYPE> ref,
                                                               @Nonnull ComponentType<ECS_TYPE, T> componentType,
                                                               @Nonnull T component) {
    this.assertThread();
    this.assertWriteProcessing();
  ...
  @Nullable
  protected <T extends Component<ECS_TYPE>> T __internal_getComponent(@Nonnull Ref<ECS_TYPE> ref, @Nonnull
  ...
  public <T extends Component<ECS_TYPE>> void tryRemoveComponent(@Nonnull Ref<ECS_TYPE> ref,
                                                                 @Nonnull ComponentType<ECS_TYPE, T> componentType) {
    this.removeComponentIfExists(ref, componentType);
  }

  public <T extends Component<ECS_TYPE>> boolean removeComponentIfExists(@Nonnull Ref<ECS_TYPE> ref, @Nonnull
  ComponentType<ECS_TYPE, T> componentType) {
    this.assertThread();
    this.assertWriteProcessing();
    ref.validate();
  ...
  public <T extends Resource<ECS_TYPE>> void replaceResource(@Nonnull ResourceType<ECS_TYPE, T> resourceType,
                                                             @Nonnull T resource) {
    this.assertThread();
  ...
  public void forEachChunk(Query<ECS_TYPE> query,
                           @Nonnull BiConsumer<ArchetypeChunk<ECS_TYPE>, CommandBuffer<ECS_TYPE>> consumer) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
      this.assertThread();
  ...
  public boolean forEachChunk(Query<ECS_TYPE> query,
                              @Nonnull BiPredicate<ArchetypeChunk<ECS_TYPE>, CommandBuffer<ECS_TYPE>> predicate) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
      this.assertThread();
  ...
  public void forEachChunk(int systemIndex,
                           @Nonnull BiConsumer<ArchetypeChunk<ECS_TYPE>, CommandBuffer<ECS_TYPE>> consumer) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
  ...
  public boolean forEachChunk(int systemIndex,
                              @Nonnull BiPredicate<ArchetypeChunk<ECS_TYPE>, CommandBuffer<ECS_TYPE>> predicate) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
          ...
          if (archetypeChunk != null) {
            ...
            if (size != 0) {
              ...
              for (int systemTaskSize = systemTask.size(); i < systemTaskSize; ++i) {
                ((ForEachTaskData) systemTask.get(i)).init(consumer, archetypeChunk, commandBuffer.fork());
              }
            }
          }
        }
  ...
  public void forEachEntityParallel(Query<ECS_TYPE> query,
  ...
  public <T extends ArchetypeDataSystem<ECS_TYPE, Q, R>, Q, R> void fetch(@Nonnull SystemType<ECS_TYPE, T> systemType,
                                                                          Q query, @Nonnull List<R> results) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
      this.assertThread();
      ...
      try {
        ...
        SystemTaskData.invokeParallelTask(this.fetchTask, commandBuffer, results);
      } finally {
        this.processing.unlock();
      }
  ...
  public <T extends EntityDataSystem<ECS_TYPE, Q, R>, Q, R> void fetch(@Nonnull Collection<Ref<ECS_TYPE>> refs,
                                                                       @Nonnull SystemType<ECS_TYPE, T> systemType,
                                                                       Q query, @Nonnull List<R> results) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
  ...
  public <Event extends EcsEvent> void invoke(@Nonnull EntityEventType<ECS_TYPE, Event> systemType,
                                              @Nonnull Ref<ECS_TYPE> ref, @Nonnull Event param) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
      this.assertThread();
  ...
  public <Event extends EcsEvent> void invoke(@Nonnull WorldEventType<ECS_TYPE, Event> systemType,
                                              @Nonnull Event param) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
  ...
  protected <Event extends EcsEvent> void internal_invoke(CommandBuffer<ECS_TYPE> sourceCommandBuffer,
                                                          Ref<ECS_TYPE> ref, Event param) {
    EntityEventType<ECS_TYPE, ?> eventType = this.registry.getEntityEventTypeForClass(param.getClass());
    if (eventType != null) {
  ...
  protected <Event extends EcsEvent> void internal_invoke(CommandBuffer<ECS_TYPE> sourceCommandBuffer,
                                                          @Nonnull EntityEventType<ECS_TYPE, Event> systemType,
                                                          Ref<ECS_TYPE> ref, Event param) {
    ComponentRegistry.Data<ECS_TYPE> data = this.registry._internal_getData();
  ...
  protected <Event extends EcsEvent> void internal_invoke(CommandBuffer<ECS_TYPE> sourceCommandBuffer,
                                                          @Nonnull WorldEventType<ECS_TYPE, Event> systemType,
                                                          Event param) {
    ComponentRegistry.Data<ECS_TYPE> data = this.registry._internal_getData();
    BitSet systemIndexes = data.getSystemIndexesForType(systemType);
    int systemIndex = -1;
  ...
  private <Tickable extends TickableSystem<ECS_TYPE>> void tickInternal(float dt,
                                                                        SystemType<ECS_TYPE, Tickable> tickingSystemType) {
    if (this.shutdown) {
      throw new IllegalStateException("Store is shutdown!");
    } else {
      this.assertThread();
  ...
  public void tick(ArchetypeTickingSystem<ECS_TYPE> system, float dt, int systemIndex) {
    if (this.shutdown) {
      ...
      try {
        ...
        com.hypixel.hytale.component.system.tick.EntityTickingSystem.SystemTaskData.invokeParallelTask(
            this.parallelTask, commandBuffer);
      } finally {
        this.processing.unlock();
        this.disableProcessingAssert = oldDisableProcessingAssert;
      }
    ...
    if (this.shutdown) {
      ...
      while ((systemIndex = systemIndexes.nextSetBit(systemIndex + 1)) >= 0) {
        ...
        if (oldSystemIndex >= 0) {
          ...
          this.systemMetrics[systemIndex] =
              HistoricMetric.builder(33333333L, TimeUnit.NANOSECONDS).addPeriod(1L, TimeUnit.SECONDS)
                  .addPeriod(1L, TimeUnit.MINUTES).addPeriod(5L, TimeUnit.MINUTES).build();
        }
      }
  ...
  private void updateData(@Nonnull ComponentRegistry.Data<ECS_TYPE> oldData,
                          @Nonnull ComponentRegistry.Data<ECS_TYPE> newData, DataChange dataChange) {
    this.processing.lock();
  ...
  private void updateData0(@Nonnull ComponentRegistry.Data<ECS_TYPE> oldData,
                           @Nonnull ComponentRegistry.Data<ECS_TYPE> newData, DataChange dataChange) {
    if (dataChange instanceof ComponentChange) {
      ...
      ComponentType<ECS_TYPE, UnknownComponents<ECS_TYPE>> unknownComponentType =
          this.registry.getUnknownComponentType();
      switch (componentChange.getType()) {
        case REGISTERED:
          String componentId = newData.getComponentId(componentType);
          Codec<Component<ECS_TYPE>> componentCodec = newData.getComponentCodec(componentType);
                ...
                if (!archetype.contains(componentType) && archetype.contains(unknownComponentType)) {
                  ...
                  archetypeChunk.transferSomeTo(tempInternalEntityHolder, toArchetypeChunk, (index) -> {
                    UnknownComponents<ECS_TYPE> unknownComponents =
                        (UnknownComponents) archetypeChunk.getComponent(index, unknownComponentType);
        ...
        case UNREGISTERED:
          ...
          Codec<Component<ECS_TYPE>> componentCodec = oldData.getComponentCodec(componentType);
          int oldArchetypeSize = this.archetypeSize;
  ...
  private void updateArchetypeIndexes(@Nonnull ComponentRegistry.Data<ECS_TYPE> data) {
    ...
    if (oldLength < systemSize) {
      this.systemIndexToArchetypeChunkIndexes =
          (BitSet[]) Arrays.copyOf(this.systemIndexToArchetypeChunkIndexes, systemSize);
  ...
  public void assertThread() {
    ...
    if (!currentThread.equals(this.thread) && this.thread.isAlive()) {
      ...
      throw new IllegalStateException(
          "Assert not in thread! " + var10002 + " but was in " + String.valueOf(currentThread));
    }
  }
  ...
  public String toString() {
    ...
    return "Store{super()=" + var10000 + "@" + this.hashCode() + ", registry=" +
        String.valueOf(this.registry.getClass()) + "@" + this.registry.hashCode() + ", shutdown=" + this.shutdown +
        ", storeIndex=" + this.storeIndex + ", systemIndexToArchetypeChunkIndexes=" +
        Arrays.toString(this.systemIndexToArchetypeChunkIndexes) + ", archetypeSize=" + this.archetypeSize +
        ", archetypeChunks=" + Arrays.toString(this.archetypeChunks) + "}";
  }

  private <T extends Component<ECS_TYPE>> void datachunk_addComponent(@Nonnull Ref<ECS_TYPE> ref,
  ...
  private int findOrCreateArchetypeChunk(@Nonnull Archetype<ECS_TYPE> archetype) {
    ...
    if (archetypeIndex != Integer.MIN_VALUE) {
      ...
      if (oldLength <= archetypeIndex) {
        ...
        this.archetypeChunkIndexesToSystemIndex =
            (BitSet[]) Arrays.copyOf(this.archetypeChunkIndexesToSystemIndex, newLength);
        int systemSize = data.getSystemSize();
